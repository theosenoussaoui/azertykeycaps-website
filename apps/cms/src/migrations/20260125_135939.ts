...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(
    sql`ALTER TABLE \`articles\` ADD \`meta_image_id\` integer REFERENCES media(id);`,
  );
  await db.run(
  ...
  await db.run(
    sql`ALTER TABLE \`keycap_profiles\` ADD \`meta_description\` text;`,
  );
  await db.run(
    sql`ALTER TABLE \`keycap_profiles\` ADD \`meta_image_id\` integer REFERENCES media(id);`,
  );
  await db.run(
    sql`CREATE INDEX \`keycap_profiles_meta_meta_image_idx\` ON \`keycap_profiles\` (\`meta_image_id\`);`,
  );
  ...
  await db.run(
    sql`ALTER TABLE \`homepage\` ADD \`meta_image_id\` integer REFERENCES media(id);`,
  );
  await db.run(
  ...
  await db.run(
    sql`ALTER TABLE \`informations_page\` ADD \`meta_image_id\` integer REFERENCES media(id);`,
  );
  await db.run(
    sql`CREATE INDEX \`informations_page_meta_meta_image_idx\` ON \`informations_page\` (\`meta_image_id\`);`,
  );
  await db.run(
    sql`ALTER TABLE \`suggestion_page\` ADD \`meta_image_id\` integer REFERENCES media(id);`,
  );
  await db.run(
    sql`CREATE INDEX \`suggestion_page_meta_meta_image_idx\` ON \`suggestion_page\` (\`meta_image_id\`);`,
  );
}
...
export async function down({
  ...
  `);
  await db.run(
    sql`INSERT INTO \`__new_keycap_profiles\`("id", "title", "abbreviation", "slug", "description", "thumbnail_id", "navbar_description", "navbar_icon_name", "shape", "updated_at", "created_at") SELECT "id", "title", "abbreviation", "slug", "description", "thumbnail_id", "navbar_description", "navbar_icon_name", "shape", "updated_at", "created_at" FROM \`keycap_profiles\`;`,
  );
  await db.run(sql`DROP TABLE \`keycap_profiles\`;`);
  await db.run(
    sql`ALTER TABLE \`__new_keycap_profiles\` RENAME TO \`keycap_profiles\`;`,
  );
  await db.run(
  ...
  `);
  await db.run(
    sql`INSERT INTO \`__new_homepage\`("id", "title", "description", "updated_at", "created_at") SELECT "id", "title", "description", "updated_at", "created_at" FROM \`homepage\`;`,
  );
  ...
  `);
  await db.run(
    sql`INSERT INTO \`__new_informations_page\`("id", "title", "content", "meta_title", "meta_description", "updated_at", "created_at") SELECT "id", "title", "content", "meta_title", "meta_description", "updated_at", "created_at" FROM \`informations_page\`;`,
  );
  await db.run(sql`DROP TABLE \`informations_page\`;`);
  await db.run(
    sql`ALTER TABLE \`__new_informations_page\` RENAME TO \`informations_page\`;`,
  );
  ...
  `);
  await db.run(
    sql`INSERT INTO \`__new_suggestion_page\`("id", "title", "description", "meta_title", "meta_description", "form_enabled", "updated_at", "created_at") SELECT "id", "title", "description", "meta_title", "meta_description", "form_enabled", "updated_at", "created_at" FROM \`suggestion_page\`;`,
  );
  await db.run(sql`DROP TABLE \`suggestion_page\`;`);
  await db.run(
    sql`ALTER TABLE \`__new_suggestion_page\` RENAME TO \`suggestion_page\`;`,
  );
}
